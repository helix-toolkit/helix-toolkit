...
public class WICHelper
{
    ...
    private static ImagingFactory? _factory = new();

    private static ImagingFactory Factory
    {
        get
        {
            return _factory ??= new ImagingFactory();
        }
    }
    ...
    //-------------------------------------------------------------------------------------
    private readonly struct WICTranslate
    {
    ...
    private readonly struct WICConvert
    {
    ...
    private static Image? DecodeMultiframe(WICFlags flags, ImageDescription metadata, BitmapDecoder decoder)
    {
        ...
        if (!ToWIC(metadata.Format, out Guid sourceGuid))
            return null;
    ...
    internal static Image? LoadFromWICMemory(IntPtr pSource, int size, bool makeACopy, GCHandle? handle)
    {
        ...
        using (var stream = new WICStream(Factory, new DataPointer(pSource, size)))
        {
            ...
            try
            {
                ...
                // Get metadata
                var tempDesc = DecodeMetadata(flags, decoder, frame, out Guid convertGuid);
    ...
    {
        if (!ToWIC(image.Format, out Guid pfGuid))
    ...
    private static Guid GetContainerFormatFromFileType(ImageFileType fileType)
    {
        ...
        {
            ImageFileType.Bmp => ContainerFormatGuids.Bmp,
            ImageFileType.Jpg => ContainerFormatGuids.Jpeg,
            ImageFileType.Gif => ContainerFormatGuids.Gif,
            ImageFileType.Png => ContainerFormatGuids.Png,
            ImageFileType.Tiff => ContainerFormatGuids.Tiff,
            ImageFileType.Wmp => ContainerFormatGuids.Wmp,
