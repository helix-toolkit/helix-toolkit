...
public sealed class Image : Component
{
    ...
    /// Pixel buffers.
    /// </summary>
    internal PixelBuffer[] pixelBuffers = Array.Empty<PixelBuffer>();
    private DataBox[] dataBoxArray = Array.Empty<DataBox>();
    private List<int> mipMapToZIndex = new();
    private int zBufferCountPerArraySlice;
    private MipMapDescription[] mipmapDescriptions = Array.Empty<MipMapDescription>();
    ...
    internal unsafe void Initialize(ImageDescription description, IntPtr dataPointer, int offset, GCHandle? handle, bool bufferIsDisposable, PitchFlags pitchFlags = PitchFlags.None)
    {
        ...
        zBufferCountPerArraySlice = this.mipMapToZIndex[^1];
    ...
    {
        return CalculateMipMapDescription(metadata, cpFlags, out int nImages, out int pixelSize);
    }
