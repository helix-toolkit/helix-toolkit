...
public class DynamicCubeMapCore : RenderCore, IDynamicReflector
{
    ...
    private CubeFaceCamerasStruct cubeFaceCameras = new() { Cameras = new CubeFaceCamera[6] };
    ...
    protected ShaderPass? DefaultShaderPass
    {
        private set
        {
            ...
            {
                cubeTextureSlot = value?.PixelShader.ShaderResourceViewMapping.TryGetBindSlot(ShaderCubeTextureName) ?? 0;
                textureSamplerSlot = value?.PixelShader.SamplerMapping.TryGetBindSlot(ShaderCubeTextureSamplerName) ?? 0;
    ...
    private bool CreateCubeMapResources()
    {
        ...
        for (var i = 0; i < 6; ++i)
        {
            ...
            cubeRTVs[i] = CubeMap?.Resource is null ? null : new RenderTargetView(Device, CubeMap.Resource, rtsDesc);
        }
