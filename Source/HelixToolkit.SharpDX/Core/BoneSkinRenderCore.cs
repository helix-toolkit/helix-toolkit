...
public class BoneSkinRenderCore : MeshRenderCore
{
    ...
    protected override bool OnAttach(IRenderTechnique? technique)
    {
        if (base.OnAttach(technique))
        {
            ...
            boneSkinSBSlot = preComputeBoneSkinPass?.VertexShader.ShaderResourceViewMapping.GetMapping(DefaultBufferNames.BoneSkinSB)?.Slot ?? 0;
            mtWeightsBSlot = preComputeBoneSkinPass?.VertexShader.ShaderResourceViewMapping.GetMapping(DefaultBufferNames.MTWeightsB)?.Slot ?? 0;
            mtDeltasBSlot = preComputeBoneSkinPass?.VertexShader.ShaderResourceViewMapping.GetMapping(DefaultBufferNames.MTDeltasB)?.Slot ?? 0;
            mtOffsetsBSlot = preComputeBoneSkinPass?.VertexShader.ShaderResourceViewMapping.GetMapping(DefaultBufferNames.MTOffsetsB)?.Slot ?? 0;
    ...
    {
        //Skip if not ready
        if (preComputeBoneSkinPass == null || preComputeBoneSkinPass.IsNULL || preComputeBoneBuffer == null || !preComputeBoneBuffer.CanPreCompute)
            return;
        ...
        if ((boneBuffer.BoneMatrices?.Length ?? 0) == 0 && !mtChanged)
        {
        ...
        else
        {
            ...
            if (GeometryBuffer?.VertexBuffer[0] is not null)
            {
    ...
    {
        return preComputeBoneBuffer?.CopySkinnedToArray(context, array) ?? 0;
    }
