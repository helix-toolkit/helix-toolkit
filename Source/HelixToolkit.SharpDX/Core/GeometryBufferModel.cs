...
public abstract class GeometryBufferModel : DisposeObject, IGUID, IGeometryBufferModel
{
    ...
    private static readonly IElementsBufferProxy[] emptyBuffers = Array.Empty<IElementsBufferProxy>();
    ...
    private static readonly VertexBufferBinding[] emptyBinding = Array.Empty<VertexBufferBinding>();
    ...
    private void Geometry_PropertyChanged(object? sender, System.ComponentModel.PropertyChangedEventArgs e)
    {
        ...
        for (var i = 0; i < VertexBuffer.Length; ++i)
        {
            if (IsVertexBufferChanged(e.PropertyName ?? string.Empty, i))
            {
                VertexChanged |= 1u << i;
                InvalidateRenderer();
                vertChanged = true;
        ...
        if (!vertChanged && IsIndexBufferChanged(e.PropertyName ?? string.Empty))
        {
            IndexChanged = true;
            InvalidateRenderer();
        }
