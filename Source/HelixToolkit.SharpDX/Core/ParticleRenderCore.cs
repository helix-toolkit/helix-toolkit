...
{
    private static readonly ILogger logger = Logger.LogManager.Create<ParticleRenderCore>();
...
#pragma warning restore
    ...
    public bool AnimateSpriteByEnergy
    {
        ...
        get
        {
            return FrameVariables.AnimateByEnergyLevel == 1;
        }
    }
    ...
    public SamplerStateDescription? SamplerDescription
    {
        set
        {
            ...
            {
                var newSampler = value is null ? null : EffectTechnique?.EffectsManager?.StateManager?.Register(value.Value);
    ...
    public bool CumulateAtBound
    {
        ...
        get
        {
            return FrameVariables.CumulateAtBound == 1;
        }
    }
    ...
    private ConstantBufferProxy? particleCountStaging
        = new("particleStaging", 4 * sizeof(int), BindFlags.None, CpuAccessFlags.Read, ResourceOptionFlags.None, ResourceUsage.Staging);
    ...
    private ParticleCountIndirectArgs drawArgument = new();
    #endregion
    ...
    private void OnUpdatePerModelStruct(RenderContext context)
    {
        ...
        modelStruct.HasInstances = (InstanceBuffer?.HasElements ?? false) ? 1 : 0;
        modelStruct.HasTexture = HasTexture ? 1 : 0;
        FrameVariables.RandomVector = VectorGenerator?.RandomVector3 ?? Vector3.Zero;
    }
    ...
    protected override bool OnAttach(IRenderTechnique? technique)
    {
        ...
        currentStateSlot = updatePass?.GetShader(ShaderStage.Compute)?.UnorderedAccessViewMapping.TryGetBindSlot(CurrentSimStateUAVBufferName) ?? 0;
        newStateSlot = updatePass?.GetShader(ShaderStage.Compute)?.UnorderedAccessViewMapping.TryGetBindSlot(NewSimStateUAVBufferName) ?? 0;

        renderStateSlot = renderPass?.GetShader(ShaderStage.Vertex)?.ShaderResourceViewMapping.TryGetBindSlot(SimStateBufferName) ?? 0;
        textureSlot = renderPass?.PixelShader.ShaderResourceViewMapping.TryGetBindSlot(ShaderTextureBufferName) ?? 0;
        samplerSlot = renderPass?.PixelShader.SamplerMapping.TryGetBindSlot(ShaderTextureSamplerName) ?? 0;
        #endregion
        if (isInitialParticleChanged)
        {
            OnInitialParticleChanged(ParticleCount);
        }
        textureSampler = SamplerDescription is null ? null : technique?.EffectsManager?.StateManager?.Register(SamplerDescription.Value);
        OnTextureChanged();
        OnBlendStateChanged();
        return true;
    }
    ...
    protected override void OnUpdate(RenderContext context, DeviceContextProxy deviceContext)
    {
        ...
        drawArgument.InstanceCount = (InstanceBuffer?.Buffer is null || !InstanceBuffer.HasElements) ? 1 : (uint)InstanceBuffer.Buffer.ElementCount;
        ...
        else
        {
            ...
            if (particleCountGSIABuffer?.Buffer is not null)
            {
        ...
        if (totalElapsed > InsertElapseThrottle && insertPass is not null)
        {
        ...
        (BufferProxies[1], BufferProxies[0]) = (BufferProxies[0], BufferProxies[1]);
    }
    /// <summary>
    /// Called when [render].
    /// </summary>
    ...
    {
        if (updatePass is null || renderPass is null)
        {
            return;
        }
    ...
    {
        if (particleCountStaging is null)
        {
            return 0;
        }
        ...
        var currentParticleCount = UnsafeHelper.Read<int>(db.Value.DataPointer);
#if OUTPUTDEBUGGING
