...
public static class IViewportExtensions
{
    ...
    [ThreadStatic]
    private static readonly Stack<IEnumerator<SceneNode>> stackCache = new();
    /// <summary>
    ...
    {
        hits?.Clear();
        if (viewport.RenderHost != null && viewport.RenderHost.RenderContext is not null)
        {
    ...
    public static bool UnProject(this IViewport3DX viewport, Vector2 point2d, out Ray ray)
    {
        ...
        {
            return renderContext.UnProject(point2d, out ray);
        }
        else
        {
            ray = new Ray();
    ...
    public static Vector2 Project(this IViewport3DX viewport, Vector3 point)
    {
        ...
        return viewport.RenderHost.RenderContext?.Project(point) ?? Vector2.Zero;
    }
    ...
    {
        Matrix? m = viewport.RenderHost != null && viewport.RenderHost.RenderContext != null ?
    ...
    public static BoundingBox FindBoundsInternal(this IViewport3DX viewport)
    {
        ...
        {
            if (r.Visible && r is not ScreenSpacedNode)
            {
                return true;
            }
            return false;
    ...
    public static System.IO.MemoryStream? RenderToBitmapStream(this IViewport3DX view)
    {
        if (view.RenderHost != null && view.RenderHost.IsRendering)
        {
            ...
            if (view.RenderHost != null && view.RenderHost.IsRendering && view.RenderHost.EffectsManager is not null)
            {
