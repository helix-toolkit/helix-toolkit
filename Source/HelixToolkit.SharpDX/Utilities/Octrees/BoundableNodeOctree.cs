...
public class BoundableNodeOctree : DynamicOctreeBase<SceneNode>
{
    ...
    public override void BuildTree()
    {
        if (IsRoot)
        {
            OctantDictionary = new Dictionary<Guid, IDynamicOctree?>(Objects?.Count ?? 0);
        }
        base.BuildTree();
        if (IsRoot)
        {
            ...
            {
                if (node is DynamicOctreeBase<SceneNode> { Objects: not null })
                {
    ...
    public IDynamicOctree? FindItemByGuid(Guid guid, SceneNode item, out int index)
    {
        ...
        if (root?.OctantDictionary is null)
        {
            index = -1;
            return null;
        }
        ...
        if (root.OctantDictionary.ContainsKey(guid))
        {
            ...
            index = (node as DynamicOctreeBase<SceneNode>)?.Objects?.IndexOf(item) ?? -1;
    ...
    {
        if (root?.OctantDictionary is not null && root.OctantDictionary.ContainsKey(guid))
        {
    ...
    public override bool Add(SceneNode item, out IDynamicOctree? octant)
    {
        if (base.Add(item, out octant))
        {
            ...
            var root = FindRoot(this) as BoundableNodeOctree;
            if (root?.OctantDictionary is not null && !root.OctantDictionary.ContainsKey(item.GUID))
            {
    ...
    public override bool PushExistingToChild(int index, out IDynamicOctree? octant)
    {
        ...
        {
            var root = FindRoot(this) as BoundableNodeOctree;
            if (root?.OctantDictionary is not null)
            {
