...
public class BoneSkinMeshNode : MeshNode, Animations.IBoneMatricesNode
{
    ...
    public Matrix[]? BoneMatrices
    {
        set
        {
            if (RenderCore is BoneSkinRenderCore core)
            {
                core.BoneMatrices = value ?? new Matrix[] { Matrix.Identity };
            }
        }
        get
        {
    ...
    public float[]? MorphTargetWeights
    {
        get
        {
            if (RenderCore is BoneSkinRenderCore core)
            {
                return core.MorphTargetWeights;
            }
    ...
    private Vector3[] skinnedVerticesCache = Array.Empty<Vector3>();
    /// <summary>
    ...
    {
        var model = EffectsManager?.GeometryBufferManager?.Register<BoneSkinnedMeshBufferModel>(modelGuid, geometry) is not IBoneSkinMeshBufferModel buffer ?
    ...
    {
        if (Geometry is BoneSkinnedMeshGeometry3D skGeometry && skGeometry.Positions is not null)
        {
            if (RenderCore is BoneSkinRenderCore skCore && manager.Device is not null)
            {
    ...
    public bool InitializeMorphTargets(MorphTargetVertex[] mtv, int pitch)
        => (RenderCore as BoneSkinRenderCore)?.InitializeMorphTargets(mtv, pitch) ?? false;
    ...
    protected override bool OnHitTest(HitTestContext? context, Matrix totalModelMatrix, ref List<HitTestResult> hits)
    {
        ...
        if (BoneMatrices is not null
            && BoneMatrices.Length > 0
            && Geometry is BoneSkinnedMeshGeometry3D skGeometry
            && skGeometry.Positions is not null
            && context.RenderMatrices?.RenderHost is not null)
        {
            if (RenderCore is BoneSkinRenderCore skCore)
            {
                ...
                if (context.RenderMatrices?.RenderHost?.ImmediateDeviceContext is not null && skCore.CopySkinnedToArray(context.RenderMatrices.RenderHost.ImmediateDeviceContext, skinnedVerticesCache) > 0)
                {
