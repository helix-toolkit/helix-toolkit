...
public class ColorStripeMaterialVariables : MaterialVariable
{
    ...
    private void CreateTextureView(IList<Color4>? colors, int which)
    {
        ...
        textures[which] = (colors == null || colors.Count == 0 || deviceResources.Device is null) ? null : new ShaderResourceViewProxy(deviceResources.Device);
        textures[which]?.CreateViewFromColorArray(colors?.ToArray() ?? Array.Empty<Color4>());
