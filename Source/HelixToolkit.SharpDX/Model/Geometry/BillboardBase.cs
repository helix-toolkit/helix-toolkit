...
{
    private static readonly ILogger logger = Logger.LogManager.Create<BillboardBase>();
        ...
        object? originalSource, int count)
    {
        ...
        var visualToScreen = context.RenderMatrices?.ScreenViewProjectionMatrix ?? Matrix.Identity;
        var screenPoint = context.HitPointSP * (context.RenderMatrices?.DpiScale ?? 1.0f);
        ...
        for (var i = 0; i < count; ++i)
        {
            ...
            var quad = GetScreenQuad(ref c, ref vert.OffTL, ref vert.OffTR, ref vert.OffBL, ref vert.OffBR, ref visualToScreen, context.RenderMatrices?.DpiScale ?? 1.0f);
        ...
        object? originalSource, int count)
    {
        ...
        var viewMatrix = context.RenderMatrices?.ViewMatrix ?? Matrix.Identity;
