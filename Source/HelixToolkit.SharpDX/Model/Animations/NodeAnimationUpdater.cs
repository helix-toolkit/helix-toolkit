...
public class NodeAnimationUpdater : IAnimationUpdater
{
    ...
    private readonly List<Model.Scene.SceneNode> animationRoots = new();
    ...
    private void UpdateBoneSkinMesh()
    {
        if (Animation.HasBoneSkinMeshes && changed)
        {
            ...
            {
                if (m.IsRenderable && !m.HasBoneGroup && m.Bones is not null) // Do not update if has a bone group. Update the group only
                {
