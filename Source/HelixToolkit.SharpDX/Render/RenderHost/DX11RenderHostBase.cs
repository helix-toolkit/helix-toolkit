...
public abstract class DX11RenderHostBase : DisposeObject, IRenderHost
{
        ...
        set
        {
            ...
            if (Set(ref effectsManager, value))
            {
                ...
                if (effectsManager != null)
                {
                    ...
                    FeatureLevel = effectsManager.Device?.FeatureLevel ?? global::SharpDX.Direct3D.FeatureLevel.Level_11_0;
                    if (IsInitialized)
                    {
                        Restart(false);
                    }
    ...
    public DX11RenderHostConfiguration RenderConfiguration { set; get; } = new DX11RenderHostConfiguration()
    {
    ...
    public bool UpdateAndRender()
    {
        if (CanRender())
        {
            ...
            if (RenderConfiguration.UpdatePerFrameData && viewport is not null && renderContext is not null)
            {
                viewport.Update(t0);
            ...
            try
            {
                if (renderBuffer is not null && renderBuffer.BeginDraw())
                {
                    OnRender(t0);
                    renderBuffer.EndDraw();

                    if (renderer?.ImmediateContext is not null && EffectsManager?.DeviceContextPool is not null)
                    {
    ...
    {
        if (!IsInitialized || renderBuffer is null || context is null)
        {
            return false;
        }
    ...
    public void StartD3D(int width, int height)
    {
        lock (lockObj)
        {
            ...
            immediateDeviceContext = effectsManager?.Device is null ? null : new DeviceContextProxy(effectsManager.Device.ImmediateContext1, effectsManager.Device);
