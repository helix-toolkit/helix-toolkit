...
public sealed class RenderContext : DisposeObject, IRenderMatrices
{
    ...
    public bool SSAOEnabled
    {
        ...
        get
        {
            return globalTransform.SSAOEnabled == 1u;
        }
    }
    /// <summary>
    ...
    public RenderContext(IRenderHost renderHost)
    {
        ...
        lightScene = renderHost.EffectsManager?.ConstantBufferPool is null ? null : new Light3DSceneShared(renderHost.EffectsManager.ConstantBufferPool);
    ...
    public void UpdatePerFrameData(bool updateGlobalTransform, bool updateLights, DeviceContextProxy deviceContext)
    {
        ...
        if (updateLights && LightScene is not null)
        {
            LightScene.LightModels.HasEnvironmentMap = SharedResource?.EnvironementMap != null;
            LightScene.LightModels.EnvironmentMapMipLevels = SharedResource?.EnvironmentMapMipLevels ?? 0;
    ...
    public ShaderResourceViewProxy? GetOffScreenTexture(OffScreenTextureType type, OffScreenTextureSize size, global::SharpDX.DXGI.Format format)
    {
        return type switch
        {
            OffScreenTextureType.RenderTarget => GetOffScreenRT(size, format),
            OffScreenTextureType.DepthStencil => GetOffScreenDS(size, format),
            _ => ShaderResourceViewProxy.Empty,
        };
    }
    /// <summary>
    ...
    public ShaderResourceViewProxy? GetOffScreenRT(OffScreenTextureSize size, global::SharpDX.DXGI.Format format)
    {
        return size switch
        {
            OffScreenTextureSize.Full => RenderHost.RenderBuffer?.FullResRenderTargetPool?.Get(format),
            OffScreenTextureSize.Half => RenderHost.RenderBuffer?.HalfResRenderTargetPool?.Get(format),
            OffScreenTextureSize.Quarter => RenderHost.RenderBuffer?.QuarterResRenderTargetPool?.Get(format),
            _ => ShaderResourceViewProxy.Empty,
        };
    }
    ...
    public ShaderResourceViewProxy? GetOffScreenRT(OffScreenTextureSize size, global::SharpDX.DXGI.Format format, out int width, out int height)
    {
        switch (size)
        {
            case OffScreenTextureSize.Full:
                width = RenderHost.RenderBuffer?.FullResRenderTargetPool?.Width ?? 0;
                height = RenderHost.RenderBuffer?.FullResRenderTargetPool?.Height ?? 0;
                return RenderHost.RenderBuffer?.FullResRenderTargetPool?.Get(format);
            case OffScreenTextureSize.Half:
                width = RenderHost.RenderBuffer?.HalfResRenderTargetPool?.Width ?? 0;
                height = RenderHost.RenderBuffer?.HalfResRenderTargetPool?.Height ?? 0;
                return RenderHost.RenderBuffer?.HalfResRenderTargetPool?.Get(format);
            case OffScreenTextureSize.Quarter:
                width = RenderHost.RenderBuffer?.QuarterResRenderTargetPool?.Width ?? 0;
                height = RenderHost.RenderBuffer?.QuarterResRenderTargetPool?.Height ?? 0;
    ...
    public ShaderResourceViewProxy? GetOffScreenDS(OffScreenTextureSize size, global::SharpDX.DXGI.Format format)
    {
        return size switch
        {
            OffScreenTextureSize.Full => RenderHost.RenderBuffer?.FullResDepthStencilPool?.Get(format),
            OffScreenTextureSize.Half => RenderHost.RenderBuffer?.HalfResDepthStencilPool?.Get(format),
            OffScreenTextureSize.Quarter => RenderHost.RenderBuffer?.QuarterResDepthStencilPool?.Get(format),
            _ => ShaderResourceViewProxy.Empty,
        };
    }
    /// <summary>
    ...
    public ShaderResourceViewProxy? GetOffScreenDS(OffScreenTextureSize size, global::SharpDX.DXGI.Format format, out int width, out int height)
    {
        ...
        {
            case OffScreenTextureSize.Full:
                width = RenderHost.RenderBuffer?.FullResDepthStencilPool?.Width ?? 0;
                height = RenderHost.RenderBuffer?.FullResDepthStencilPool?.Height ?? 0;
                return RenderHost.RenderBuffer?.FullResDepthStencilPool?.Get(format);
            case OffScreenTextureSize.Half:
                width = RenderHost.RenderBuffer?.HalfResDepthStencilPool?.Width ?? 0;
                height = RenderHost.RenderBuffer?.HalfResDepthStencilPool?.Height ?? 0;
                return RenderHost.RenderBuffer?.HalfResDepthStencilPool?.Get(format);
            case OffScreenTextureSize.Quarter:
                width = RenderHost.RenderBuffer?.QuarterResDepthStencilPool?.Width ?? 0;
                height = RenderHost.RenderBuffer?.QuarterResDepthStencilPool?.Height ?? 0;
