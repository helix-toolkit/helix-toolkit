...
public class DeferredRenderer : IDisposable
{
    ...
    private Color4 ambientLightColor = new(0.2f, 0.2f, 0.2f, 1.0f);
    ...
    internal void InitBuffers(IRenderHost? host, Format format = Format.R32G32B32A32_Float)
    {
        ...
        this.format = format;
        this.targetWidth = Math.Max((int)(host?.ActualWidth ?? 0), 100);
        this.targetHeight = Math.Max((int)(host?.ActualHeight ?? 0), 100);

        // variable containers
        this.deferredLightingVariables = renderHost is null ? null : new DeferredLightingVariables(renderHost.EffectsManager, renderHost.RenderTechniquesManager);
