...
namespace HelixToolkit.Wpf.SharpDX
{
    public class OctreeLineGeometryModel3D : CompositeModel3D
    {
        public static readonly DependencyProperty OctreeProperty
            = DependencyProperty.Register("Octree", typeof(IStaticOctree), typeof(OctreeLineGeometryModel3D),
                new PropertyMetadata(null, (s, e) =>
                {
                    var d = (s as OctreeLineGeometryModel3D);
                    if (e.OldValue != null)
                    {
                        (e.OldValue as IStaticOctree).OnHit -= d.OctreeLineGeometryModel3D_OnHit;
                    }
                    if (e.NewValue != null)
                    {
                        (e.NewValue as IStaticOctree).OnHit += d.OctreeLineGeometryModel3D_OnHit;
                    }
                    d.CreateOctreeLines();
                }));
        ...
        public IStaticOctree Octree
        {
            ...
            get
            {
                return (IStaticOctree)GetValue(OctreeProperty);
            }
        }
        ...
        {
            var node = sender as IStaticOctree;
