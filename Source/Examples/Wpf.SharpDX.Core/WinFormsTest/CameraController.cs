...
namespace HelixToolkit.SharpDX.Controls
{
    using Cameras;
    ...
    public sealed class CameraController
    {
        ...
        public void ZoomByChangingFieldOfView(float delta)
        {
            ...
            if (viewport.CameraCore is PerspectiveCameraCore pcamera)
            {
                ...
                fov *= 1f + (delta * 0.5f);
                if (fov < MinimumFieldOfView)
                {
                    fov = MinimumFieldOfView;
                }

                if (fov > MaximumFieldOfView)
                {
                    fov = MaximumFieldOfView;
                }
