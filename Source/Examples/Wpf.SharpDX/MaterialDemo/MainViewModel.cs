...
{
    public ObservableElement3DCollection Model1 { get; } = new();

    public ObservableElement3DCollection Model2 { get; } = new();

    public ObservableElement3DCollection Model3 { get; } = new();

    public ObservableElement3DCollection Model4 { get; } = new();

    public ObservableElement3DCollection Model5 { get; } = new();

    public ObservableElement3DCollection Model6 { get; } = new();

    public ObservableElement3DCollection Model7 { get; } = new();

    public ObservableElement3DCollection ModelNormalVector { get; } = new();
    ...
    public MainViewModel()
    {
        ...
        if (FloorMaterial is PhongMaterial material)
        {
            material.RenderShadowMap = true;
        }
    }

    [RelayCommand]
    private void OpenPBRSample()
    {
    ...
    public void AttachModelList(List<Object3D>? objs)
    {
        ...
        for (int i = 0; i < objs.Count; ++i)
        {
            ...
            if (ob.Geometry is not null)
            {
                ob.Geometry.Colors = ob.Geometry.Positions is null ? null : new Color4Collection(Enumerable.Repeat(vertColor, ob.Geometry.Positions.Count));
            ...
            context?.Post((o) =>
            {
                ...
                {
                    PhongMaterial phong = p.ConvertToPhongMaterial()!;
