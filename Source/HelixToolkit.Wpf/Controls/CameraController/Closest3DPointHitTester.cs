...
internal sealed class Closest3DPointHitTester
{
    ...
    public IEnumerable<ClosestVertexResult> FindClosestHits(Point pointToHitTest)
    {
        ...
        mViewPort3D.Children.Traverse<GeometryModel3D>(
            (model, transform) =>
            {
                ...
                ClosestVertexResult hitTestResult = new(model, geometry, closestPoint, closestPointIn2D, Math.Sqrt(minSquaredDistanceToPoint));
