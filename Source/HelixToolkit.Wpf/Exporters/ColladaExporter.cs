...
/// </summary>
public sealed class ColladaExporter : Exporter<XmlWriter>
{
    /// <summary>
    ...
    private void ExportGeometry(XmlWriter writer, GeometryModel3D model, Transform3D transform)
    {
        ...
        if (mg == null)
        {
            ThrowHelper.ThrowInvalidOperationException("Model is not a MeshGeometry3D.");
        }
    ...
    private void ExportNode(XmlWriter writer, GeometryModel3D gm, Transform3D transform)
    {
        ...
        if (gm.Material != null && this.materials.TryGetValue(gm.Material, out string? frontMaterialId))
        {
            this.BindMaterial(writer, geometryId, frontMaterialId);
        }

        if (gm.BackMaterial != null && this.materials.TryGetValue(gm.BackMaterial, out string? backMaterialId))
        {
