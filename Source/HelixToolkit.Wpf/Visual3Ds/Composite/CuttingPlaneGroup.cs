...
public class CuttingPlaneGroup : RenderingModelVisual3D
{
    ...
    /// </summary>
    private Dictionary<Model3D, Geometry3D?> newCutGeometries = new();

    /// <summary>
    /// The new original geometries.
    /// </summary>
    private Dictionary<Model3D, Geometry3D?> newOriginalGeometries = new();
    ...
    private void ApplyCuttingGeometries(bool forceUpdate = false)
    {
        lock (this)
        {
            this.newCutGeometries = new();
            this.newOriginalGeometries = new();
    ...
    private void ApplyCuttingPlanesToModel(GeometryModel3D model, Transform3D transform, bool updateRequired)
    {
        ...
        if (this.cutGeometries.TryGetValue(model, out Geometry3D? cutGeometry))
        {
        ...
        if (!this.originalGeometries.TryGetValue(model, out Geometry3D? originalGeometry))
        {
           ...
           )
        {
            ...
            switch (this.Operation)
            {
                ...
                case CuttingOperation.Subtract:
                    var builder = new MeshBuilder(originalMeshGeometry.Normals?.Any() ?? false, originalMeshGeometry.TextureCoordinates?.Any() ?? false);
