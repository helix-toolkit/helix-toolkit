    ...
    protected virtual HxScene.SceneNode OnCreateHxMeshNode(MeshInfo mesh, HelixInternalScene scene, Matrix transform)
    {
        switch (mesh.Type)
        {
            case PrimitiveType.Triangle:
                ...
                var cullMode = material is not null && material.Value.Key.HasTwoSided && material.Value.Key.IsTwoSided
                    ? CullMode.Back
                    : CullMode.None;
                if (Configuration.ForceCullMode)
                    cullMode = Configuration.CullMode;
                var fillMode = material is not null && material.Value.Key.HasWireFrame && material.Value.Key.IsWireFrameEnabled
                    ? FillMode.Wireframe
                    : FillMode.Solid;

                //Determine if has bones
                HxScene.MeshNode mnode;
                if ((mesh.AssimpMesh?.HasBones ?? false) || (mesh.AssimpMesh?.HasMeshAnimationAttachments ?? false))
                {
