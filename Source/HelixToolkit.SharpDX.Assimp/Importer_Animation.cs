...
public partial class Importer
{
    ...
    private ErrorCode LoadAnimations(HelixInternalScene scene)
    {
        ...
        if (scene.AssimpScene is not null && scene.AssimpScene.HasAnimations)
        {
            bool hasBoneSkinnedMesh = scene.Meshes is null ? false : scene.Meshes.Where(x => x.Mesh is BoneSkinnedMeshGeometry3D).Any();
        ...
        out List<HxAnimations.Animation> hxAnis)
    {
        ...
        if (ani.MeshMorphAnimationChannelCount > 0)
        {
            ...
            {
                HxAnimations.Animation hxAni = new(HxAnimations.AnimationType.MorphTarget)
                {
                    StartTime = 0,
                    EndTime = (float)(ani.DurationInTicks / ani.TicksPerSecond),
                    Name = ani.Name,
