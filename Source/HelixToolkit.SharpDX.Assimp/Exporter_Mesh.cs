...
public partial class Exporter
{
    ...
    /// <returns></returns>
    protected virtual bool GetGeometryFromNode(HxScene.SceneNode node, [NotNullWhen(true)] out Geometry3D? geometry)
    {
    ...
    private MeshInfo? OnCreateMeshInfo(HxScene.GeometryNode geoNode)
    {
        ...
        if (geoNode is HxScene.MaterialGeometryNode materialNode && materialNode.Material != null)
        {
            ...
            {
                info = geoNode.Geometry is null ? null : new MeshInfo(key, geoNode.Geometry, geoNode.Name, geoIndex, materialIndex);
            }
            else
            {
                info = existing;
    ...
    {
        if (node.Geometry is not null && geometryCollection.TryGetValue(node.Geometry, out geoIndex))
        {
    ...
    protected virtual Mesh OnCreateAssimpMesh(MeshInfo info)
    {
        ...
        if (info.Mesh is MeshGeometry3D mesh)
        {
            ...
            if (info.Bones != null &&
                ...
                && boneSkinMesh.VertexBoneIds is not null && boneSkinMesh.Positions is not null
