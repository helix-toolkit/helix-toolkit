...
public sealed class Rect3D
{
    ...
    /// </returns>
    public PlaneIntersectionType Intersects(Vector3 planePosition, Vector3 planeNormal)
    {
        ...
        Vector3 absPlaneNormal = new(Math.Abs(planeNormal.X), Math.Abs(planeNormal.Y), Math.Abs(planeNormal.Z));
        Vector3 center = this.GetCenter();
        Vector3 centerToCorner = center - this.Location;
        float extents = Vector3.Dot(centerToCorner, absPlaneNormal);
        float distance = center.DistanceToPlane(planePosition, planeNormal);
