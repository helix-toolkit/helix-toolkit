...
public static class CameraExtensions
{
    ...
    {
        if (source is not IProjectionCameraModel projectionSource || dest is not IProjectionCameraModel projectionDest)
        {
            return;
        }
    ...
    {
        if (camera is not IProjectionCameraModel projectionCamera)
        {
            return default;
        }

        var axis1 = Vector3D.Cross(projectionCamera.LookDirection, projectionCamera.UpDirection);
        var axis2 = Vector3D.Cross(axis1, projectionCamera.LookDirection);
        axis1 = Vector3.Normalize(axis1);
        axis2 = Vector3.Normalize(axis2);
        var l = projectionCamera.LookDirection.Length();
        var f = l * 0.001;
        var move = (-axis1 * (float)(f * dx)) + (axis2 * (float)(f * dy));
    ...
    /// </returns>
    public static Matrix GetInverseViewProjectionMatrix3D(this Camera camera, double aspectRatio)
    {
        return GetInverseViewProjectionMatrix(camera, aspectRatio);
    }
    ...
    public static Matrix GetProjectionMatrix3D(this Camera camera, double aspectRatio)
    {
        return GetProjectionMatrix(camera, aspectRatio);
    }
    ...
    {
        return camera?.CreateProjectionMatrix((float)aspectRatio) ?? Matrix.Identity;
    }
    ...
    public static Matrix GetViewProjectionMatrix3D(this Camera camera, double aspectRatio)
    {
        return GetViewProjectionMatrix(camera, aspectRatio);
    }
    ...
    /// </returns>
    public static Matrix GetViewMatrix3D(this Camera camera)
    {
    ...
    {
        return camera?.CreateViewMatrix() ?? Matrix.Identity;
    }
    ...
    {
        LookAt(camera, target.ToVector3(), camera.LookDirection, animationTime);
    }
#endif
    ...
    public static void LookAt(this Camera camera, Point3D target, double distance, double animationTime)
    {
        ...
        LookAt(camera, target, d * (float)distance, animationTime);
    }
        ...
        this Camera camera, Viewport3DX viewport, Point3D center, double radius, double animationTime = 0)
    {
        ...
        if (camera is IPerspectiveCameraModel pcam)
        {
            ...
            LookAt(camera, center, dir * (float)dist, animationTime);
        }
