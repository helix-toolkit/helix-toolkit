...
public abstract class GroupElement3D : Element3D
{
    ...
    public static readonly DependencyProperty ItemsSourceProperty =
        HelixProperty.Register<GroupElement3D, IEnumerable<Element3D>?>("ItemsSource",
            null,
                (d, e) =>
                {
    ...
    public static readonly DependencyProperty OctreeManagerProperty =
        HelixProperty.Register<GroupElement3D, IOctreeManagerWrapper?>("OctreeManager",
            null, (s, e) =>
        {
            if (s is not GroupElement3D d)
            {
                return;
            }
    ...
    public static readonly DependencyProperty AlwaysHittableProperty =
        HelixProperty.Register<GroupElement3D, bool>("AlwaysHittable",
            false, (d, e) =>
        {
            if (d is GroupElement3D element)
            {
                element.SceneNode.AlwaysHittable = (bool)e.NewValue!;
            }
        });
    ...
    private void OnItemsSourceChanged(IEnumerable<Element3D>? itemsSource)
    {
        ...
        if (itemsSourceInternal != null)
        {
            ...
            {
                s.CollectionChanged -= Source_CollectionChanged;
            }
        }
    ...
    private void Source_CollectionChanged(object? sender, NotifyCollectionChangedEventArgs e)
    {
        ...
        {
            case NotifyCollectionChangedAction.Reset:
                if (itemsSourceInternal is not null)
                {
                    foreach (var item in itemsSourceInternal)
                    {
                        Children.Add(item);
