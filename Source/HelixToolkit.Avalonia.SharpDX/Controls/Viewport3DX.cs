...
public partial class Viewport3DX : TemplatedControl, IViewport3DX
{
    public static bool IsInDesignMode => Design.IsDesignMode;

    public double ActualWidth => Bounds.Width * DpiScale;

    public double ActualHeight => Bounds.Height * DpiScale;
    ...
    private void SetupBindings()
    {
        ...
        {
            new(ViewportCommands.ZoomExtents, this.ZoomExtentsHandler),
            new(ViewportCommands.SetTarget, this.cameraController.setTargetHandler!.Execute),
            new(ViewportCommands.Reset, this.ResetHandler),
            new(ViewportCommands.Zoom, this.cameraController.zoomHandler!.Execute),
            new(ViewportCommands.Pan, this.cameraController.panHandler!.Execute),
            new(ViewportCommands.Rotate, this.cameraController.rotateHandler!.Execute),
            new(ViewportCommands.ChangeFieldOfView, this.cameraController.changeFieldOfViewHandler!.Execute),
            new(ViewportCommands.ZoomRectangle, this.cameraController.zoomRectangleHandler!.Execute),
            new(ViewportCommands.BottomView, this.BottomViewHandler),
            new(ViewportCommands.TopView, this.TopViewHandler),
            new(ViewportCommands.FrontView, this.FrontViewHandler),
            new(ViewportCommands.BackView, this.BackViewHandler),
            new(ViewportCommands.LeftView, this.LeftViewHandler),
            new(ViewportCommands.RightView, this.RightViewHandler)
        };
    ...
    protected override void OnApplyTemplate(TemplateAppliedEventArgs e)
    {
        ...
        hostPresenter = e.NameScope.Find<ContentPresenter>(ViewportPartNames.PartCanvas);
        ...
        if (hostPresenter is not null)
        {
            ...
            hostPresenter.Content = new D3DDrawingSurfaceBase()
            {
                DpiScale = DpiScale
            };
        }
        ...
        if (this.hostPresenter?.Content is not null)
        {
        ...
        if (this.renderHostInternal != null)
        {
            ...
            this.renderHostInternal.IsRendering = this.IsVisible;
        ...
        this.coordinateView ??= e.NameScope.Find<ScreenSpacedElement3D>(ViewportPartNames.PartCoordinateView);
        ...
        this.viewCube ??= e.NameScope.Find<ScreenSpacedElement3D>(ViewportPartNames.PartViewCube);
        if (this.viewCube == null)
        {
            throw new HelixToolkitException("{0} is missing from the template.", ViewportPartNames.PartViewCube);
        }
        this.frameStatisticModel ??= e.NameScope.Find<FrameStatisticsModel2D>(ViewportPartNames.PartFrameStatisticView);
        ...
        this.partItemsControl ??= e.NameScope.Find<ItemsControl>(ViewportPartNames.PartItems);
        ...
        this.LogicalChildren.Remove(Overlay2D);
        this.LogicalChildren.Add(Overlay2D);
        var titleView = e.NameScope.Find(ViewportPartNames.PartTitleView);
    ...
    /// The animation time.
    /// </param>
    public void ZoomExtents(BoundingBox bounds, double animationTime = 0)
    {
    ...
    public void EmulateMouseDownByTouch(PointerPressedEventArgs e)
    {
        this.Focus();
        this.MouseDownHitTest(e.GetPosition(this), e);
    }
    ...
    public void EmulateMouseUpByTouch(PointerEventArgs e)
    {
        this.MouseUpHitTest(e.GetPosition(this), e);
    }
    ...
    /// </summary>
    private void CameraPropertyChanged(AvaloniaPropertyChangedEventArgs e)
    {
    ...
    {
        var bindingExpression = BindingOperations.GetBindingExpressionBase(this, RenderExceptionProperty);
    ...
    private void ClearDefaultGestures()
    {
        ...
        this.PointerPressed -= DefaultGesturesPointerPressed;
    }
    ...
    private void SetDefaultGestures()
    {
        ...
        var kb = new[]
        {
                new KeyBinding() {Command = ViewportCommands.TopView, Gesture = new(Key.U)},
                new KeyBinding() {Command = ViewportCommands.BottomView, Gesture = new(Key.D)},
                new KeyBinding() {Command = ViewportCommands.FrontView, Gesture = new(Key.F)},
                new KeyBinding() {Command = ViewportCommands.BackView, Gesture = new(Key.B)},
                new KeyBinding() {Command = ViewportCommands.LeftView, Gesture = new(Key.L)},
                new KeyBinding() {Command = ViewportCommands.RightView, Gesture = new(Key.R)},
            };
        this.KeyBindings.AddRange(kb);

        this.KeyBindings.Add(new KeyBinding() { Command = ViewportCommands.ZoomExtents, Gesture = new(Key.E, KeyModifiers.Control) });

        this.PointerPressed += DefaultGesturesPointerPressed;
    }

    private void DefaultGesturesPointerPressed(object? sender, PointerPressedEventArgs e)
    {
        KeyModifiers modifiers = e.KeyModifiers;
        PointerPointProperties buttons = e.GetCurrentPoint(null).Properties;
        int clicks = e.ClickCount;

        if (buttons.IsLeftButtonPressed && clicks == 2 && modifiers.HasFlag(KeyModifiers.Control))
        {
            ViewportCommands.ZoomExtents.Execute(e, this);
        }

        if (buttons.IsRightButtonPressed && modifiers == KeyModifiers.None)
        {
            ViewportCommands.Rotate.Execute(e, this);
        }

        if (buttons.IsRightButtonPressed && modifiers == KeyModifiers.Control)
        {
            ViewportCommands.Zoom.Execute(e, this);
        }

        if (buttons.IsRightButtonPressed && modifiers == KeyModifiers.Shift)
        {
            ViewportCommands.Pan.Execute(e, this);
        }

        if (buttons.IsRightButtonPressed && modifiers == KeyModifiers.Alt)
        {
            ViewportCommands.ChangeFieldOfView.Execute(e, this);
        }

        if (buttons.IsRightButtonPressed && modifiers == (KeyModifiers.Control & KeyModifiers.Shift))
        {
            ViewportCommands.ZoomRectangle.Execute(e, this);
        }

        if (buttons.IsRightButtonPressed && clicks == 2 && modifiers == KeyModifiers.Control)
        {
            ViewportCommands.SetTarget.Execute(e, this);
        }

        if (buttons.IsMiddleButtonPressed && clicks == 2 && modifiers == KeyModifiers.Control)
        {
            ViewportCommands.Reset.Execute(e, this);
        }
    }
    ...
    public bool HittedSomething(PointerEventArgs e)
    {
    ...
    /// </param>
    private void MouseDownHitTest(Point pt, PointerEventArgs? originalInputEventArgs = null)
    {
        ...
        if (this.FindHits(pt.ToVector2(), ref hits))
        {
            ...
            //if (this.touchDownDevice == null)
            {
                ...
                originalInputEventArgs?.Pointer.Capture(this);
            }
    ...
    private bool ViewBoxHitTest(Point p, UIInputEventArgs? originalInputEventArgs = null)
    {
    ...
    /// The original input event (which mouse button pressed?)
    /// </param>
    private void MouseUpHitTest(Point pt, PointerEventArgs? originalInputEventArgs = null)
    {
        if (!enableMouseButtonHitTest)
        {
            return;
    ...
    public static T? FindVisualAncestor<T>(StyledElement obj) where T : StyledElement
    {
        if (obj != null)
        {
