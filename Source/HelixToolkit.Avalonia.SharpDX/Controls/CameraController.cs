...
public class CameraController
{
    ...
    public Cursor ChangeFieldOfViewCursor = new Cursor(StandardCursorType.SizeNorthSouth);
    ...
    public Cursor PanCursor = new Cursor(StandardCursorType.Hand);
    ...
    public Cursor RotateCursor = new Cursor(StandardCursorType.SizeAll);
    ...
    public Cursor ZoomCursor = new Cursor(StandardCursorType.SizeNorthSouth);

    /// <summary>
    /// Gets or sets the zoom rectangle cursor.
    /// </summary>
    /// <value> The zoom rectangle cursor. </value>
    public Cursor ZoomRectangleCursor = new Cursor(StandardCursorType.BottomRightCorner);
    ...
    {
        ChangeDirection(lookDir.ToVector3(), upDir.ToVector3(), animationTime);
    }
#endif
    ...
    /// </param>
    /// <param name="e">
    /// The <see cref="PointerWheelEventArgs"/> instance containing the event data.
    /// </param>
    public void OnMouseWheel(object? sender, PointerWheelEventArgs e)
    {
        ...
        if (this.ZoomAroundMouseDownPoint)
        {
            ...
            {
                this.AddZoomForce(-(float)e.Delta.X * 0.001f, nearestPoint);
                e.Handled = true;
                return;
            }
        }

        this.AddZoomForce(-(float)e.Delta.X * 0.001f);
        e.Handled = true;
    }
