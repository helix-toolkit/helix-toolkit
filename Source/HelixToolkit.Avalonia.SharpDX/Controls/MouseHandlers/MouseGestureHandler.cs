...
internal abstract class MouseGestureHandler
{
    ...
    public void Execute(object? sender, ExecutedRoutedEventArgs e)
    {
        ...
        PointerEventArgs p = (PointerEventArgs)e.Parameter!;

        this.Viewport.PointerMoved -= this.OnMouseMove;
        this.Viewport.PointerPressed -= this.OnMouseUp;
        this.Viewport.PointerPressed += this.OnMouseUp;
        this.Viewport.Focus();
        // toto: MouseCapture
        //this.Viewport.CaptureMouse();
        p.Pointer.Capture(this.Viewport);
        this.OnMouseDown(sender, p);
        this.Viewport.PointerMoved += this.OnMouseMove;
    }
    ...
    public virtual void Started(Point e)
    {
        ...
        startTick = Stopwatch.GetTimestamp();
        Inv = Camera is not null && Camera.CreateLeftHandSystem ? -1 : 1;
    ...
    {
        return this.Viewport.UnProject(position.ToVector2());
    }
    ...
    /// </param>
    /// <param name="e">
    /// The <see cref="PointerEventArgs"/> instance containing the event data.
    /// </param>
    protected virtual void OnMouseDown(object? sender, PointerEventArgs? e)
    {
        if (e is not null)
        {
            this.Started(e.GetPosition(this.Viewport));
        }
    ...
    protected virtual void OnMouseMove(object? sender, PointerEventArgs e)
    {
        if (e.Handled)
            return;
        this.Delta(e.GetPosition(this.Viewport));
    }
    ...
    /// The event arguments.
    /// </param>
    protected virtual void OnMouseUp(object? sender, PointerPressedEventArgs e)
    {
        this.Viewport.PointerMoved -= this.OnMouseMove;
        this.Viewport.PointerPressed -= this.OnMouseUp;
        // toto: MouseCapture
        ...
        this.Viewport.Cursor = Controller.CursorHistory.Count > 0 ? Controller.CursorHistory.Pop() : new Cursor(StandardCursorType.Arrow);
        this.Completed(e.GetPosition(this.Viewport));
        CheckCursorHistory();
    }
